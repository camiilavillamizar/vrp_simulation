                ...
                if (res->amount > 0 && !claimed[y][x]) {
                    if ((res->type == CELL_WOOD && needed_wood) ||
                        ...
                        if (selector == 0) score = -d; // Greedy: nearest
                            ...
                            int* kx = (int*)malloc(K_NEAREST * sizeof(int));
                            int* ky = (int*)malloc(K_NEAREST * sizeof(int));
                            int* kd = (int*)malloc(K_NEAREST * sizeof(int));
                            int kn = 0;

                            // printf("[Tick %d][Villager %d] Searching K Nearest Resources from (%d, %d) of type %d...\n",
                            //     tick, tid, start_x, start_y, res->type);
                                ...
                                for (int xx = 0; xx < game_map.width; ++xx) {
                                    ...
                                    if (r->amount > 0 && r->type == res->type) {
                                        ...
                                        if (kn < K_NEAREST) {
                                            ...
                                            ky[kn] = yy;
                                            kd[kn] = dd;
                                            // printf("  Added candidate (%d, %d) with amount %d, distance %d\n", xx, yy, r->amount, dd);
                                            kn++;
                                        } else {
                                            int max_idx = 0;
                                            for (int j = 1; j < K_NEAREST; ++j)
                                                if (kd[j] > kd[max_idx]) max_idx = j;

                                            if (dd < kd[max_idx]) {
                                                // printf("  Replacing (%d, %d) with (%d, %d) — better distance %d < %d\n",
                                                //     kx[max_idx], ky[max_idx], xx, yy, dd, kd[max_idx]);
                                                kx[max_idx] = xx;
                                                ky[max_idx] = yy;
                                                kd[max_idx] = dd;
                                            }
                            ...
                            for (int j = 0; j < kn; ++j) {
                                ...
                                // printf("  Checking candidate (%d, %d) with amount %d\n", kx[j], ky[j], rr->amount);
                                if (rr->amount > max_cap) {
                                    max_cap = rr->amount;
                                    best_x = kx[j];
                                    best_y = ky[j];
                                    best_type = res->type;
                            ...
                            if (max_cap > 0) {
                                take = (max_cap > left) ? left : max_cap;
                            }
